...
В проекте используются Server Actions Next.js (`"use server"`).

### Правила:

- Server Actions **пишутся через `function`**, а не стрелочные функции.
- Это исключение сделано намеренно:
  - для соответствия Next.js канонам,
  - для корректной сериализации,
  - для читаемых stack traces.

### Где разрешены `function`:
